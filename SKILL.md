...
| [references/constructibles.md](references/constructibles.md) | Buildings/wonders as Constructibles + TypeTags: the **AGELESS** tag, obsolescence (Age-computed), **overbuilding** (building→building only — wonders can't), making a building age-bound, the density-vs-wonders tile tension. |
...
> **Generate the local data references first.** Three references — `effects-collections-catalog.md`,
> `display-names.md`, `progression-trees.md` — are **not bundled** (they're bulk extractions of the base game's
> own data/text). Generate them from your installed copy of the game (one-time, and after each patch):
> ```
